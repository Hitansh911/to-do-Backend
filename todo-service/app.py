...
def get_todos():
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT id, title, due_date, priority, completed FROM todos WHERE user_id = %s", (user_id,))
        rows = cur.fetchall()
        data = [{"id": r[0], "title": r[1], "due_date": str(r[2]), "priority": r[3], "completed": r[4]} for r in rows]
...
def create_todo():
    ...
    try:
        cur = conn.cursor()
        cur.execute("INSERT INTO todos (user_id, title, due_date, priority) VALUES (%s, %s, %s, %s) RETURNING id",
                    (user_id, data.get('title'), data.get('due_date'), data.get('priority', 'low')))
        todo_id = cur.fetchone()[0]
        conn.commit()
        redis_client.delete(f"todos:{user_id}")
        return jsonify({"status": "success", "data": {"id": todo_id}}), 201
    finally:
        db_pool.putconn(conn)
